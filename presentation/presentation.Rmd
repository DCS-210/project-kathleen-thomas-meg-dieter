...
ggplot(data = data_set %>% filter(YEAR == 2015)) +
  geom_point(mapping = aes(x = SUPPORT_SERVICES_EXPENDITURE, 
...
ggplot(data = AL_MA_data) + 
  geom_line(mapping = aes(x = YEAR, y = STATE_REVENUE, color = STATE)) +
  labs(
    x = "Year",
    y = "State Revenue",
    title = "State Revenue",
...
```{r instruction-expenditure, echo = FALSE, warning = FALSE}
ggplot(data = data_set %>% filter(YEAR == 2015)) +
  geom_point(mapping = aes(x = INSTRUCTION_EXPENDITURE, 
                           y = AVG_READING_8_SCORE, 
                           color = STATE)) + 
  labs(title = 'Instruction Expenditure and Average 8th Grade Reading Score', x = 'Instruction Expenditure', y = 'Average 8th Grade Reading Score') + 
  theme(legend.title = element_text(size = 10), 
               legend.text = element_text(size = 7))
=======
```
...
ggplot(data = AL_MA_data) + 
  ...
  labs(
    ...
    subtitle = "Alabama and Massachusetts") +
 guides(col=guide_legend("State"))
>>>>>>> bd6cad36e98e72c760f3f708df3ad70336c1b7c2
```
